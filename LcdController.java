...
public final class LcdController implements Component, Clocked {
    ...
    public LcdImage currentImage() {
        return currentImage == null ?
                new LcdImage.Builder(160, 144).build() : currentImage;
    }
    ...
    public int read(int address) {
        ...
        } else if (address >= REGS_LCDC_START
    ...
    private void computeLine(int y) {
        ...
        if (lcdBank.testBit(Reg.LCDC, LcdcBits.OBJ)) {
            ...
            LcdImageLine spritesBGLine = extractSpritesLine(y, spritesList, true);
            LcdImageLine spritesFGLine = extractSpritesLine(y, spritesList, false);
            BitVector bgOpacity = bgLine.opacity().or(spritesBGLine.opacity().not());
    ...
    private LcdImageLine extractSpritesLine(int y, int[] spritesList, boolean background) {
        ...
        for (int i = 0; i < spritesList.length; i++) {
            int spriteMemoryIndex = OAM_START + 4 * spritesList[spritesList.length - 1 - i]; // bonne méthode ?
    ...
    private int[] spritesIntersectingLine(int y) {
        ...
        for (int i = 0; i < 40; i++) { // magic numbers
            ...
            if (nbOfSprites < sprites.length && y >= spriteOrdinate && y < spriteOrdinate + size)
    ...
    private int getTileLineVector(int tileSourceStart, int tileIndex, int tileLineIndex, boolean msb) {
